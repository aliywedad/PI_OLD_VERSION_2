...
def formInfo(request):
    ...
    for form in forms:
        ...
        for village in villages:
            ...
            for date in reponsesDates:
                ...
                infalist=[]
                questionsList = []
                infravillage=InfrastructuresVillage.objects.filter(date_info=date['response_date'],NumeroVillage=village)
                reponses = Reponse.objects.filter(village=village,response_date=date['response_date'])

                try:
                    for reponse in reponses:
                        if reponse.question_id.formilair==form:
    ...
    #         return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
                ...
                if len(reponsesList):
                    user=''
                    try :
                        user=reponse.id_user.fullname()
                    except:
                        print("user not found")
                    questions_data={ "Date":date['response_date'].strftime("%Y-%m-%d %H:%M:%S"),"user":user,"data":reponsesList ,"commune": village.idCommit.Nom_commune,"village":village.NomAdministratifVillage,"moghataa":village.idCommit.ID_maghataa_id.Nom_maghataa,"id_formulaire":form.id,"wilaya":village.idCommit.ID_maghataa_id.ID_wilaya.Nom_wilaya}
                    listdata.append(questions_data)
        form_data={ "formilaire":form.titre,'questions': listdata}
...
def auth(request):
    ...
    # password=make_password(passwordsimple)
    # hashed_password = hashlib.md5(password.encode()).hexdigest()
    # print("password",make_password("1234"))
    # print("password",make_password("1234"))
    
    if email is None or password is None:
        return JsonResponse({"message": "Email or password is missing"}, status=409)

    try:
        # userConn += email
        user = users.objects.get(email=email)  # Use correct model class here
        print(user.password)
        print(password)
        # print(hashed_password)
    except :
        return JsonResponse({"message": "User Not Found"}, status=410)

    if user.password != password:
        return JsonResponse({"message": "Incorrect Password"}, status=411)
    if not user.active  :
        return JsonResponse({"message": "Unauthorized Access"}, status=412)
  
    if user.role != "admin":
        return JsonResponse({"message": "Unauthorized Access"}, status=413)

    # ActionUser.objects.create(Action=f"L'utilisateur {userConn} s'est connecté avec succès",date=gmt_time,user=user)
    # return JsonResponse({"message": "Unauthorized Access"}, status=412)
    
    # ActionUser.objects.create(Action=f"L'utilisateur {userConn} s'est connecté avec succès",date=gmt_time,user=user.id)
    user.password=""
